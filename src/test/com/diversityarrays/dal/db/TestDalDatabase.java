...
public class TestDalDatabase {
	...
	private static final boolean WANT_TIMING = Boolean
			.getBoolean(TestDalDatabase.class.getSimpleName() + ".WANT_TIMING");
	private static final boolean NOISY = Boolean
			.getBoolean(TestDalDatabase.class.getSimpleName() + ".NOISY");

	private static final boolean WANT_JSON = true;

	// Set true to get first login to display messages
	private boolean firstLogin = Boolean.getBoolean(TestDalDatabase.class
			.getSimpleName() + ".firstLogin");
	...
	static {
		...
		CENTRAL_LOCAL_URL_PAIRS.put("BMAC", new String[] {
				"jdbc:mysql://localhost:3306/iwis_gms?user=root", null // "jdbc:mysql://localhost:3306/iwis_gms?user=root"
				});

		CENTRAL_LOCAL_URL_PAIRS.put("HOME", new String[] {
				"jdbc:mysql://localhost:3306/iwis_gms?user=root", null // "jdbc:mysql://localhost:4406/iwis_gms?user=root"
				});
	}
	...
	// try {
	// conn =
	// DbUtil.createConnection("jdbc:mysql://localhost:4406/iwis_gms?user=root",
	// null, null, null);
	...
	static OperationMatch getOperationMatch(String dalcmd) {
		...
		OperationMatch match = DalDatabaseUtil.findOperationMatch(dalcmd,
				wordNodeRoot, errmsg);
	...
	public static void setUpBeforeClass() throws Exception {
		...
		String testDbNameProperty = TestDalDatabase.class.getSimpleName()
				+ ".TEST_DBNAME";
		...
		if ("BMS".equalsIgnoreCase(testDbName)) {
			...
			throw new RuntimeException("Unsupported value '" + testDbName
					+ "' for " + testDbNameProperty);
		}
	...
	private static DalDatabase createBMS_DalDatabase() throws UnknownHostException, DalDbException {
		...
		if (hostname.startsWith("192.168.9.")) {
			...
			if (hostname.startsWith("pyrus")) {
				where = "HOME";
					...
					|| hostname.startsWith("bmac") || h2.startsWith("beepsmac")) {
				where = "BMAC";
			} else {
				where = "DART";
			}
		...
		JdbcConnectionParameters centralParams = new JdbcConnectionParameters(
				centralLocal[0], null, null);
		JdbcConnectionParameters localParams = null;
		if (centralLocal[1] != null && !centralLocal[1].isEmpty()) {
			localParams = new JdbcConnectionParameters(centralLocal[1], null,
					null);
		}

		JdbcConnectionParameters interopParams = new JdbcConnectionParameters(
				"jdbc:mysql://172.17.60.87:13306/siu_bms", "bmsadmin",
				"bms;2015*");

		@SuppressWarnings("unchecked")
		DalDatabase result = new BMS_DalDatabase(ClosureUtils.nopClosure(),
				false, localParams, centralParams,
				BMSApiDataConnection.BMS_USER,
				BMSApiDataConnection.BMS_PASSWORD, interopParams);

		System.out.println(TestDalDatabase.class.getName() + " for KDDart: "
	...
	static private DalDatabase createKddartDalDatabase() throws DalDbException {
		...
		Preferences preferences = Preferences
				.userNodeForPackage(DalServer.class);
		...
		Preferences serviceNode = preferences.node("service/"
		...
		KddartDalDatabase result = (KddartDalDatabase) service.createDatabase(
				parameterValues.values(), ClosureUtils.nopClosure(), false);
	...
	private void doListGroup(String testName, DalSession session) {
		DalResponseBuilder responseBuilder = DalServerUtil
				.createBuilder(WANT_JSON);
	...
	public void testListOperation() {
		...
		Collections.sort(list, new Comparator<DalOperation>() {
			...
			public int compare(DalOperation o1, DalOperation o2) {
				return o1.getCommandTemplate().compareTo(
						o2.getCommandTemplate());
			}
	...
	final private void doLoggedInTest(LoggedInTest loggedInTest) {
		...
		try {
			...
			UserInfo userInfo = dalDatabase.doLogin(newSessionId, USERNAME,
					seo, parms);
			...
			if (WANT_TIMING) {
				System.err.println(loggedInTest.name + ": loginTime="
						+ (loginNanos / 1_000_000.0) + " ms, testTime="
	...
	public void testGetGenus() {
		...
		LoggedInTest loggedInTest = new LoggedInTest("getGenus", true) {
			...
			public void execute(DalSession session) {
				...
				try {
					match.node.getOperation().execute(session, responseBuilder,
							Method.GET, dalcmd, match.getParameterValues(),
							null, null);
	...
	private void checkJsonResult(String where,
			DalResponseBuilder responseBuilder, String... keysExpected) {
		...
		try {
			...
			if (responseKeys.size() != (keysExpected.length + 1)) {
				fail(where + " : expected key count mismatch: "
						+ responseKeys.size() + "<>"
						+ (keysExpected.length + 1) + "\n\tactual= "
						+ StringUtil.join(",", responseKeys)
			...
			keySet.add(DALClient.TAG_RECORD_META); // must ALWAYS expect
													// 'RecordMeta'
			...
			if (!keySet.isEmpty()) {
				fail(where + ": missing keys in response: "
						+ StringUtil.join(",", keySet));
			}
			...
			if (!keySet.isEmpty()) {
				fail(where + ": unexpected keys in response: "
						+ StringUtil.join(",", keySet));
			}

			if (checkPagination) {
				Object paginationObject = jsonMap.get(DALClient.TAG_PAGINATION);
				assertNotNull("Missing tag '" + DALClient.TAG_PAGINATION + "'",
						paginationObject);

				if (!(paginationObject instanceof List)) {
					fail(DALClient.TAG_PAGINATION + " is not a List: "
							+ paginationObject.getClass().getName());
				}

				@SuppressWarnings("rawtypes")
				List list = (List) paginationObject;
				assertEquals(DALClient.TAG_PAGINATION
						+ " is not a List of size 1", 1, list.size());

				Object mapObject = list.get(0);
				assertEquals(DALClient.TAG_PAGINATION + "[0] is not a JsonMap",
						JsonMap.class, mapObject.getClass());

				JsonMap pagination = (JsonMap) mapObject;

				Set<String> paginationKeys = new HashSet<String>(
						pagination.getKeysInOrder());

				for (String attrName : PAGINATION_ATTRIBUTES) {
					Object attrObject = pagination.get(attrName);

					assertNotNull("Missing attribute "
							+ DALClient.TAG_PAGINATION + "." + attrName,
							attrObject);

					assertEquals(DALClient.TAG_PAGINATION + "." + attrName
							+ " is not a String", String.class,
							attrObject.getClass());

					try {
						Integer.parseInt((String) attrObject);
					} catch (NumberFormatException e) {
						fail(DALClient.TAG_PAGINATION + "." + attrName
	...
	static private final String[] PAGINATION_ATTRIBUTES = {
			DALClient.ATTR_PAGE, DALClient.ATTR_NUM_OF_RECORDS,
			DALClient.ATTR_NUM_OF_PAGES, DALClient.ATTR_NUM_PER_PAGE };
	...
	public void testListGenus() {
		...
		LoggedInTest loggedInTest = new LoggedInTest("listGenus", true) {
			...
			public void execute(DalSession session) {
				...
				try {
					match.node.getOperation().execute(session, responseBuilder,
							Method.GET, dalcmd, null, null, null);
		...
		LoggedInTest loggedInTest = new LoggedInTest("getGenotypeAlias", true) {
			...
			public void execute(DalSession session) {
				...
				try {
					...
					checkJsonResult("testGetGenotypeAlias", responseBuilder,
							"GenotypeAlias");
	...
	public void testGenotypeListAlias() {
		...
		LoggedInTest loggedInTest = new LoggedInTest("genotypeListAlias", true) {
			...
			public void execute(DalSession session) {
				...
				String dalcmd = "genotype/" + LIST_GENOTYPE_ALIAS_ID
						+ "/list/alias";
				...
				try {
					...
					checkJsonResult("testGenotypeListAlias," + dalcmd,
							responseBuilder, "GenotypeAlias");
		...
		LoggedInTest loggedInTest = new LoggedInTest("listGenotypeAlias", true) {
			...
			public void execute(DalSession session) {
				...
				try {
					...
					checkJsonResult("testListGenotypeAlias", responseBuilder,
							DALClient.TAG_PAGINATION, "GenotypeAlias");
					if (NOISY) {
		...
		LoggedInTest loggedInTest = new LoggedInTest("getGenotype", true) {
			...
			public void execute(DalSession session) {
				...
				try {
					...
					checkJsonResult("testGetGenotype", responseBuilder,
							"Genotype");
		...
		LoggedInTest loggedInTest = new LoggedInTest("listGenotype", true) {
			...
			public void execute(DalSession session) {
				...
				try {
					...
					checkJsonResult("testListGenotype", responseBuilder,
							DALClient.TAG_PAGINATION, "Genotype");
					if (NOISY) {
	...
	static class FilteringTest {
		...
		FilteringTest(String input, String countClause, String getClause,
				String listAlias, String pagedList) {
			//
			this.input = input;
	...
	public void testGenotypeAliasFiltering() {
		...
		FilteringTest[] GENOTYPE_ALIAS_FACTORY_FILTER_CLAUSES = {
				new FilteringTest("GenotypeAliasId = 7",
						"(nstat!=9) AND ( nid = 7 )", "(nstat!=9) AND (nid="
								+ GET_GENOTYPE_ALIAS_ID + ") AND ( nid = 7 )",
						"(nstat!=9) AND (gid=" + GET_GENOTYPE_ID
								+ ") AND ( nid = 7 ) LIMIT 20 OFFSET 1",
						"(nstat!=9) AND ( nid = 7 ) LIMIT 20 OFFSET 1"),

				new FilteringTest("GenotypeAliasName = 'ABC'",
						"(nstat!=9) AND ( nval = 'ABC' )",
						"(nstat!=9) AND (nid=" + GET_GENOTYPE_ALIAS_ID
								+ ") AND ( nval = 'ABC' )",
						"(nstat!=9) AND (gid=" + GET_GENOTYPE_ID
								+ ") AND ( nval = 'ABC' ) LIMIT 20 OFFSET 1",
						"(nstat!=9) AND ( nval = 'ABC' ) LIMIT 20 OFFSET 1"),

				new FilteringTest(
						"GenotypeAliasName LIKE '%abc%'",
						"(nstat!=9) AND ( nval LIKE '%abc%' )",
						"(nstat!=9) AND (nid=" + GET_GENOTYPE_ALIAS_ID
								+ ") AND ( nval LIKE '%abc%' )",
						"(nstat!=9) AND (gid="
								+ GET_GENOTYPE_ID
								+ ") AND ( nval LIKE '%abc%' ) LIMIT 20 OFFSET 1",
						"(nstat!=9) AND ( nval LIKE '%abc%' ) LIMIT 20 OFFSET 1"),

				new FilteringTest("GenotypeAliasStatus = 23",
						"(nstat!=9) AND ( nstat = 23 )", "(nstat!=9) AND (nid="
								+ GET_GENOTYPE_ALIAS_ID
								+ ") AND ( nstat = 23 )",
						"(nstat!=9) AND (gid=" + GET_GENOTYPE_ID
								+ ") AND ( nstat = 23 ) LIMIT 20 OFFSET 1",
				...
				new FilteringTest("GenotypeAliasLang = 'Chinese'",
						...
						"(nstat!=9) AND (nid=" + GET_GENOTYPE_ALIAS_ID
								+ ") AND ( nstat IN (3,4) )",
						"(nstat!=9) AND (gid=" + GET_GENOTYPE_ID
								+ ") AND ( nstat IN (3,4) ) LIMIT 20 OFFSET 1",
		...
		for (FilteringTest ftest : GENOTYPE_ALIAS_FACTORY_FILTER_CLAUSES) {
			...
			try {
				...
				checkEquals("genotypeAliasFactory.createCountQuery",
						ftest.countClause, sql, failures);
				if (NOISY) {
					System.out.println("testGenotypeAliasFiltering."
							+ ".createCountQuery=" + sql);
				}

				sql = genotypeAliasFactory.createGetQuery(
						GET_GENOTYPE_ALIAS_ID, filterClause);
				checkEquals("genotypeAliasFactory.createGetQuery",
						ftest.getClause, sql, failures);
				if (NOISY) {
					System.out.println("testGenotypeAliasFiltering."
							+ ".createGetQuery=" + sql);
				}

				sql = genotypeAliasFactory.createListAliasQuery(
						GET_GENOTYPE_ID, 1, 20, filterClause);
				checkEquals("genotypeAliasFactory.createListAliasQuery",
						ftest.listAlias, sql, failures);
				if (NOISY) {
					System.out.println("testGenotypeAliasFiltering."
							+ ".createListAliasQuery=" + sql);
				}

				sql = genotypeAliasFactory.createPagedListQuery(1, 20,
						filterClause);
				checkEquals("genotypeAliasFactory.createPagedListQuery",
						ftest.pagedList, sql, failures);
				if (NOISY) {
					System.out.println("testGenotypeAliasFiltering."
							+ ".createPagedListQuery=" + sql);
				}

				if (!failures.isEmpty()) {
					StringBuilder sb = new StringBuilder(
							"testGenotypeAliasFiltering");
	...
	private void checkEquals(String testName, String expected, String sql,
		...
		if (!expected.equals(afterWhere)) {
			failures.add(testName + ": got '" + afterWhere + "' but expected '"
					+ expected + "'");
		}
	}

	private void showResponse(String heading, DalResponseBuilder responseBuilder)
		...
		LoggedInTest loggedInTest = new LoggedInTest("getGeneralType", true) {
			...
			public void execute(DalSession session) {
				...
				try {
					...
					checkJsonResult("testGetGeneralType", responseBuilder,
							"GeneralType");
	...
	public void testSetGeneralType() {
		...
		LoggedInTest loggedInTest = new LoggedInTest("setGeneralType", true) {
			...
			public void execute(DalSession session) {
				...
				try {
					match.node.getOperation().execute(session, responseBuilder,
							Method.POST, dalcmd, null, parametros, null);

					checkJsonResult("testSetGeneralType", responseBuilder,
							"GeneralType");
		...
		LoggedInTest loggedInTest = new LoggedInTest("getItemUnit", true) {
			...
			public void execute(DalSession session) {
				...
				try {
					...
					checkJsonResult("testGetItemUnit", responseBuilder,
							"ItemUnit");
	...
	public void testGetTrial() {
		...
		LoggedInTest loggedInTest = new LoggedInTest("trial", true) {
			...
			public void execute(DalSession session) {
				...
				// String dalcmd = "list/trial/" + 1 + "/page/" + 1 +
				// "?Filtering=ProjectId%3D63c50ebb-67d1-4b8d-a237-1b9fb6d28859";
				String dalcmd = "list/trial/" + 3 + "/page/" + 1
						+ "?Filtering=Season%3D2015 Winter";
				Map<String, String> methodParams = new HashMap<String, String>();
				System.out.println("FilteringClause>>>>>>"
						+ dalcmd.split("\\?")[1]);
				...
				try {
					match.node.getOperation().execute(session, responseBuilder,
							Method.GET, dalcmd, match.getParameterValues(),
							methodParams, null);
					checkJsonResult("testGetTrial", responseBuilder,
							DALClient.TAG_PAGINATION, "Trial");
	...
	public void testSetObservation() {
		...
		LoggedInTest loggedInTest = new LoggedInTest("observation", true) {
			...
			@Override
			public void execute(DalSession session) {
				String dalcmd = "observation/" + "wheat" + "/" + "2147" + "/"
						+ "2152";
				...
				mapaParametros
						.put("json",
								"{\"TrialUnitId\":2152,\"SpecimenId\":4,\"SpecimenName\":\"CML165\",\"UnitPositionText\":4,\"entryType\":\"Test Entry\",\"UnitPositionId\":1,\"ReplicateNumber\":1,\"TreatmentId\":1,\"TrialUnitNote\":\"\",\"measurements\":[{\"measurementIdentifier\":{\"TrialTraitId\":2152,\"trait\":{\"TraitId\":18020,\"TraitName\":\"Plant_height\"}},\"TraitValue\":\"403\"}]}");

				Map<String, String> filePathByName = new HashMap<String, String>();
				filePathByName
						.put("postData",
								"{\"TrialUnitId\":2152,\"SpecimenId\":5398211,\"SpecimenName\":\"CGSS01B00062T-099Y-099M-099M-099Y-099M-12Y-0MEX\",\"UnitPositionText\":1,\"entryType\":\"Test Entry\",\"UnitPositionId\":1,\"ReplicateNumber\":1,\"TreatmentId\":1,\"TrialUnitNote\":\"Test\",\"measurements\":[{\"measurementIdentifier\":{\"TrialTraitId\":6553,\"trait\":{\"TraitId\":29096,\"TraitName\":\"HNECRO_0_1\"}},\"TraitValue\":\"999\"}]}");

				// The following example is where measurementId hasn't assigned
				// yet
				// filePathByName.put("postData",
				...
				try {
					...
					match.node.getOperation().execute(session, responseBuilder,
							...
							dalOpParameters, mapaParametros, filePathByName);

					checkJsonResult("testSetObservation", responseBuilder,
							"Observation");
					if (NOISY) {
		...
		LoggedInTest loggedInTest = new LoggedInTest("project", true) {
			...
			public void execute(DalSession session) {
				...
				try {
					...
					System.out.println("Reponse[testGetProject]"
							+ responseBuilder.asString());
					// checkJsonResult("testGetProject", responseBuilder,
					// "Project");
					if (NOISY) {
	...
	public void testGetLocations() {
		LoggedInTest loggedInTest = new LoggedInTest("location", true) {
			...
			@Override
			public void execute(DalSession session) {
				String dalcmd = "list/site/" + 4 + "/page/" + 1;
				...
				try {
					...
					checkJsonResult("testGetLocations", responseBuilder,
							DALClient.TAG_PAGINATION, "Site");
					if (NOISY) {
		...
		LoggedInTest loggedInTest = new LoggedInTest("trialType", true) {
			...
			public void execute(DalSession session) {
				...
				try {
					...
					// checkJsonResult("testGetTrialType", responseBuilder,
					// "TrialType");
					if (NOISY) {
		...
		LoggedInTest loggedInTest = new LoggedInTest("sample", true) {
			...
			public void execute(DalSession session) {
				...
				try {
					...
					// checkJsonResult("testGetSample", responseBuilder,
					// "GeneralType");
					if (NOISY) {
		...
		LoggedInTest loggedInTest = new LoggedInTest("itemParent", true) {
			...
			public void execute(DalSession session) {
				...
				try {
					...
					System.out.println("Reponse" + responseBuilder.asString());
					// checkJsonResult("testGetItemParent", responseBuilder,
					// "ItemParent");
					if (NOISY) {
		...
		LoggedInTest loggedInTest = new LoggedInTest("unitType", true) {
			...
			public void execute(DalSession session) {
				...
				try {
					...
					System.out.println("Reponse" + responseBuilder.asString());
					// checkJsonResult("testGetUnitType", responseBuilder,
					// "UnitType");
					if (NOISY) {
		...
		LoggedInTest loggedInTest = new LoggedInTest("state", true) {
			...
			public void execute(DalSession session) {
				...
				try {
					...
					System.out.println("Reponse" + responseBuilder.asString());
					// checkJsonResult("testGetState", responseBuilder,
					// "State");
					if (NOISY) {
		...
		LoggedInTest loggedInTest = new LoggedInTest("container", true) {
			...
			public void execute(DalSession session) {
				...
				try {
					...
					System.out.println("Reponse" + responseBuilder.asString());
					// checkJsonResult("testGetContainer", responseBuilder,
					// "Container");
					if (NOISY) {
		...
		LoggedInTest loggedInTest = new LoggedInTest("specimengroup", true) {
			...
			public void execute(DalSession session) {
				...
				try {
					...
					System.out.println("Reponse" + responseBuilder.asString());
					// checkJsonResult("testGetSpecimenGroup", responseBuilder,
					// "SpecimenGroup");
					if (NOISY) {
		...
		LoggedInTest loggedInTest = new LoggedInTest("parent", true) {
			...
			public void execute(DalSession session) {
				...
				try {
					...
					System.out.println("Reponse" + responseBuilder.asString());
					// checkJsonResult("testGetParent", responseBuilder,
					// "Parent");
					if (NOISY) {
		...
		LoggedInTest loggedInTest = new LoggedInTest("genotypea", true) {
			...
			public void execute(DalSession session) {
				...
				try {
					...
					System.out.println("Reponse" + responseBuilder.asString());
					// checkJsonResult("testGetGenotypeA", responseBuilder,
					// "GenotypeaAlias");
					if (NOISY) {
	...
	public void testGetGenotypeAliasStatus() {
		LoggedInTest loggedInTest = new LoggedInTest("genotypealiasstatus",
				true) {
			...
			public void execute(DalSession session) {
				...
				try {
					...
					System.out.println("Reponse" + responseBuilder.asString());
					// checkJsonResult("testGetGenotypeAliasStatus",
					// responseBuilder, "GenoTypeAliasStatus");
					if (NOISY) {
						showResponse("testGetGenotypeAliasStatus",
								responseBuilder);
					}
				} catch (DalDbException e) {
					fail(e.getMessage());
		...
		LoggedInTest loggedInTest = new LoggedInTest("genparent", true) {
			...
			public void execute(DalSession session) {
				...
				try {
					...
					System.out.println("Reponse" + responseBuilder.asString());
					// checkJsonResult("testGetGenParent", responseBuilder,
					// "GenParent");
					if (NOISY) {
		...
		LoggedInTest loggedInTest = new LoggedInTest("genotypespecimen", true) {
			...
			public void execute(DalSession session) {
				...
				try {
					...
					System.out.println("Reponse" + responseBuilder.asString());
					// checkJsonResult("testGetGenotypeSpecimen",
					// responseBuilder, "GenoTypeSpecimen");
					if (NOISY) {
		...
		LoggedInTest loggedInTest = new LoggedInTest("trialevent", true) {
			...
			public void execute(DalSession session) {
				...
				try {
					...
					System.out.println("Reponse" + responseBuilder.asString());
					// checkJsonResult("testGetTrialEvent", responseBuilder,
					// "TrialEvent");
					if (NOISY) {
	...
	public void testGetWorkflow() {
		LoggedInTest loggedInTest = new LoggedInTest("workflow", true) {
			...
			public void execute(DalSession session) {
				...
				DalResponseBuilder responseBuilder = DalServerUtil.createBuilder(false); //RHT borrar
				...
				try {
					...
					System.out.println("Reponse" + responseBuilder.asString());
					// checkJsonResult("testGetWorkflow", responseBuilder,
					// "Workflow");
					if (NOISY) {
		...
		LoggedInTest loggedInTest = new LoggedInTest("season", true) {
			...
			public void execute(DalSession session) {
				...
				try {
					...
					System.out.println("Reponse [testGetSeason]: "
							+ responseBuilder.asString());
	 ...
	 */
	@Test
	public void testGetGeneralUnit(){
		LoggedInTest loggedInTest = new LoggedInTest("generalUnit", true) {

			@Override
			public void execute(DalSession session) {

				String dalcmd = "list/generalunit/" + 1000 + "/page/" + 1;
				...
				try{
					...
					System.out.println("Reponse [testGetGeneralUnit] \n" + responseBuilder.asString() + "\n");

					if (NOISY) {
						showResponse("testGetGeneralUnit", responseBuilder);
					}
				}catch (DalDbException e) {
					fail(e.getMessage());
				}catch (ParseException e) {
	 ...
	 */
	@Test
	public void testGetTriallUnit(){
		LoggedInTest loggedInTest = new LoggedInTest("trialUnit", true) {

			@Override
			public void execute(DalSession session) {

				String dalcmd = "trial/15935/list/trialunit";
				...
				try{
					...
					System.out.println("Reponse [testGetTriallUnit] \n" + responseBuilder.asString() + "\n");

					if (NOISY) {
						showResponse("testGetTriallUnit", responseBuilder);
					}
				}catch (DalDbException e) {
					fail(e.getMessage());
				}catch (ParseException e) {
	...
	public void testGetTraits(){
		LoggedInTest loggedInTest = new LoggedInTest("traits", true) {

			@Override
			public void execute(DalSession session) {

				String dalcmd = "trial/15935/list/trait";
				...
				try{
					...
					System.out.println("Reponse [testGetTraits] \n" + responseBuilder.asString() + "\n");

					if (NOISY) {
						showResponse("testGetTraits", responseBuilder);
					}
				}catch (DalDbException e) {
					fail(e.getMessage());
				}catch (ParseException e) {
	...
	public void testGetCSV(){
		LoggedInTest loggedInTest = new LoggedInTest("csv", true) {
			...
			public void execute(DalSession session) {
				...
				try{
					...
					System.out.println("Reponse [testGetCSV] \n" + responseBuilder.asString() + "\n");

					if (NOISY) {
						showResponse("testGetCSV", responseBuilder);
					}
				}catch (DalDbException e) {
					fail(e.getMessage());
				}catch (ParseException e) {
